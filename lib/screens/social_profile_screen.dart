...
class _SocialProfileScreenState extends State<SocialProfileScreen> {
      ...
      child: Column(children: [
        Stack(clipBehavior: Clip.none, children: [
          SizedBox(
            ...
            child: cover.isEmpty
                ? Container(color: const Color(0xFFE7EEF9))
                : Image.network(cover, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(color: const Color(0xFFE7EEF9))),
          ),
          Positioned(
            left: 18,
            bottom: -48,
            child: Container(
              ...
              child: SocialAvatar(imageUrl: photo, radius: 48),
            ),
          ),
        ]),
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (bio.isNotEmpty) ...[
              ...
              Text(bio, style: GoogleFonts.inter(fontSize: 11, color: SocialTheme.muted, height: 1.4)),
            ],
            const SizedBox(height: 14),
            Row(children: [
              _count('followers', 'Pengikut'),
              _count('following', 'Mengikuti'),
              Expanded(child: Column(children: [
                Text(rating > 0 ? rating.toStringAsFixed(1) : '—', style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w900)),
                Text('Rating Rekber', style: GoogleFonts.inter(fontSize: 9, color: SocialTheme.muted)),
              ])),
            ]),
            const SizedBox(height: 12),
            own
                ? Row(children: [
                    Expanded(child: OutlinedButton.icon(onPressed: () {}, icon: const Icon(Icons.edit_outlined), label: const Text('Edit Profil'))),
                ...
                : Row(children: [
                    Expanded(child: OutlinedButton.icon(
                      onPressed: () async {
                        await SocialService.instance.toggleFollow(profileUid);
                        await _refreshFollow();
                      },
                      icon: Icon(following == true ? Icons.check : Icons.person_add_alt_1),
                      label: Text(following == true ? 'Mengikuti' : 'Ikuti'),
                    )),
                    const SizedBox(width: 8),
                    Expanded(child: FilledButton.icon(
                      onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => DirectChatScreen(otherUid: profileUid, title: name))),
  ...
  Widget _count(String collection, String label) => Expanded(
    child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: FirebaseFirestore.instance.collection('social_profiles').doc(profileUid).collection(collection).snapshots(),
      builder: (_, snapshot) => Column(children: [
        Text('${snapshot.data?.docs.length ?? 0}', style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w900)),
        Text(label, style: GoogleFonts.inter(fontSize: 9, color: SocialTheme.muted)),
      ]),
    ),
  );
  ...
  Widget _postCard(QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    ...
    return SocialCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SocialInteractionBar(
        ...
        onLike: () => SocialService.instance.toggleLike(doc.id, authorId: (data['authorId'] ?? '').toString()),
        onComment: () {},
        onShare: () => SocialService.instance.sharePost(doc.id, authorId: (data['authorId'] ?? '').toString()),
      ),
    ]));
  }
  ...
  SliverList _posts() => SliverList(
    delegate: SliverChildBuilderDelegate(
      (context, index) => Padding(padding: const EdgeInsets.only(bottom: 10), child: _postCard(_postDocs[index])),
      childCount: _postDocs.length,
    ),
  );

  List<QueryDocumentSnapshot<Map<String, dynamic>>> _postDocs = <QueryDocumentSnapshot<Map<String, dynamic>>>[];

  Widget _productsContent(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) => GridView.builder(
    shrinkWrap: true,
    ...
    itemBuilder: (context, i) {
      ...
      return SocialCard(padding: EdgeInsets.zero, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(padding: const EdgeInsets.all(9), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text((data['name'] ?? 'Produk').toString(), maxLines: 2, overflow: TextOverflow.ellipsis, style: GoogleFonts.inter(fontWeight: FontWeight.w800, fontSize: 12)),
          const SizedBox(height: 4),
          Text('Rp${data['price'] ?? 0}', style: GoogleFonts.inter(color: SocialTheme.blue, fontWeight: FontWeight.w900)),
          const SizedBox(height: 6),
          SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const HomeScreen())), icon: const Icon(Icons.shield_outlined, size: 15), label: const Text('Belanja / Rekber'))),
        ])),
      ]));
    },
  );
  ...
  Widget _products() => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
    ...
    builder: (_, snapshot) {
      ...
      if (docs.isEmpty) return SliverToBoxAdapter(child: _empty('Belum ada produk jualan aktif.'));
      return SliverToBoxAdapter(child: _productsContent(docs));
    },
  );

  Widget _postsView() => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
    ...
    builder: (_, snapshot) {
      ...
      if (docs.isEmpty) return SliverToBoxAdapter(child: _empty('Belum ada postingan.'));
  ...
  // Keamanan: data post tetap stream dari Firestore dan tidak pernah diisi dummy.
  Widget _posts() => _postsView();

  void _menu(BuildContext context) {
    showModalBottomSheet(
      context: context,
      builder: (_) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: own ? [
