...
class _SocialProfileScreenState extends State<SocialProfileScreen> {
  ...
  @override
  void initState() { super.initState(); _refreshFollow(); }

  Future<void> _refreshFollow() async {
    if (own || profileUid.isEmpty) return;
    final v = await SocialService.instance.isFollowing(profileUid);
    if (mounted) setState(() => following = v);
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (_, s) => Text((s.data?.data()?['displayName'] ?? widget.userName ?? 'Profil').toString(), style: GoogleFonts.inter(fontWeight: FontWeight.w800)),
        ),
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        stream: FirebaseFirestore.instance.collection('social_profiles').doc(profileUid).snapshots(),
        builder: (context, s) {
          final p = s.data?.data() ?? <String, dynamic>{};
          final name = (p['displayName'] ?? widget.userName ?? 'Pengguna').toString();
          return CustomScrollView(
            slivers: [
              SliverToBoxAdapter(child: _header(p, name)),
  ...
  Widget _actions(String name) {
    if (own) {
      return Row(children: [
        Expanded(child: OutlinedButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const EditProfileScreen())), icon: const Icon(Icons.edit_outlined), label: const Text('Edit Profil'))),
        const SizedBox(width: 8),
        Expanded(child: FilledButton.icon(onPressed: () => _menu(context), icon: const Icon(Icons.shield_outlined), label: const Text('Rekber'))),
      ]);
    }
    return Row(children: [
      Expanded(child: OutlinedButton.icon(onPressed: () async { await SocialService.instance.toggleFollow(profileUid); await _refreshFollow(); }, icon: Icon(following == true ? Icons.check : Icons.person_add_alt_1), label: Text(following == true ? 'Mengikuti' : 'Ikuti'))),
      const SizedBox(width: 8),
      Expanded(child: FilledButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => DirectChatScreen(otherUid: profileUid, title: name))), icon: const Icon(Icons.chat_bubble_outline), label: const Text('Kirim Pesan'))),
    ]);
  }
  ...
  Widget _posts() {
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (_, s) {
        ...
        return SliverList(delegate: SliverChildBuilderDelegate((_, i) => Padding(padding: const EdgeInsets.fromLTRB(12, 10, 12, 0), child: _post(docs[i])), childCount: docs.length));
      },
    );
  }

  Widget _post(QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    final d = doc.data();
    final imgs = (d['imageUrls'] as List?)?.map((e) => e.toString()).where((e) => e.isNotEmpty).toList() ?? <String>[];
    return SocialCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      SocialPostHeader(avatar: (d['authorPhotoUrl'] ?? '').toString(), name: (d['authorName'] ?? 'Pengguna').toString(), time: '', onMore: () => _postMenu(doc.id, (d['authorId'] ?? '').toString())),
      if ((d['text'] ?? '').toString().trim().isNotEmpty) ...[const SizedBox(height: 5), Text(d['text'].toString(), style: GoogleFonts.inter(fontSize: 13, height: 1.45))],
      if (imgs.isNotEmpty) ...[const SizedBox(height: 10), SocialImageGrid(images: imgs)],
      SocialInteractionBar(likes: (d['likeCount'] as num?)?.toInt() ?? 0, comments: (d['commentCount'] as num?)?.toInt() ?? 0, onLike: () => SocialService.instance.toggleLike(doc.id, authorId: (d['authorId'] ?? '').toString()), onComment: () {}, onShare: () => SocialService.instance.sharePost(doc.id, authorId: (d['authorId'] ?? '').toString())),
    ]));
  }
          ...
          sliver: SliverGrid(
            delegate: SliverChildBuilderDelegate((context, i) {
              ...
              return SocialCard(padding: EdgeInsets.zero, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Expanded(child: url.isEmpty ? const Center(child: Icon(Icons.image_outlined, size: 42)) : Image.network(url, width: double.infinity, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Center(child: Icon(Icons.broken_image_outlined)))),
                Padding(padding: const EdgeInsets.all(9), child: Text((d['name'] ?? 'Produk').toString(), maxLines: 2, overflow: TextOverflow.ellipsis, style: GoogleFonts.inter(fontWeight: FontWeight.w800, fontSize: 12))),
              ]));
  ...
  void _postMenu(String id, String authorId) {
    showModalBottomSheet(context: context, builder: (_) => SafeArea(child: ListTile(leading: Icon(authorId == uid ? Icons.delete_outline : Icons.flag_outlined), title: Text(authorId == uid ? 'Hapus postingan' : 'Laporkan postingan'), onTap: () async { Navigator.pop(context); if (authorId == uid) await FirebaseFirestore.instance.collection('posts').doc(id).delete(); })));
  }

  void _menu(BuildContext context) {
    final items = own ? <Widget>[
      ListTile(leading: const Icon(Icons.edit_outlined), title: const Text('Edit Profil'), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const EditProfileScreen()))),
    ...
    showModalBottomSheet(context: context, builder: (_) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: items)));
  }
}
