...
class ChatScreen extends StatefulWidget {
  final String title;
  const ChatScreen({super.key, this.title = 'Chat Admin'});
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  CollectionReference<Map<String, dynamic>> get msgs => FirebaseFirestore.instance
      .collection('chats')
      .doc(uid)
      .collection('messages');
  ...
  Future<void> pick() async {
    try {
      ...
      _err('Gagal memilih gambar: $e');
    }
  }

  Future<void> send() async {
    final body = text.text.trim();
    if (uid.isEmpty || sending || (body.isEmpty && image == null)) return;
    ...
    try {
      ...
      if (file != null) {
        final reference = FirebaseStorage.instance.ref(
          'chat/$uid/${DateTime.now().millisecondsSinceEpoch}.jpg',
        );
      ...
      _err('Pesan gagal dikirim. Periksa koneksi/izin Firebase.\n$e');
    } finally {
        ...
        children: [
          Expanded(
            child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              stream: msgs.orderBy('createdAt').snapshots(),
              builder: (context, snapshot) {
                if (snapshot.hasError) {
                  return Center(child: Text('Chat belum dapat dimuat: ${snapshot.error}'));
                }
                final docs = snapshot.data?.docs ?? const [];
                if (docs.isEmpty) {
                  return const Center(
                    child: Text(
                      'Belum ada pesan.\nKirim pesan atau foto untuk menghubungi Admin.',
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (body.isNotEmpty)
                              Padding(
                                padding: EdgeInsets.only(top: url.isNotEmpty ? 8 : 0),
                                child: Text(
                                  body,
                                  style: TextStyle(
              ...
              child: Row(
                children: [
                  IconButton(
                    ...
                    icon: const Icon(Icons.photo_outlined, color: blue),
                  ),
                  Expanded(
                    child: TextField(
                      controller: text,
