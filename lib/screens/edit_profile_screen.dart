...
/// Editor profil sosial: nama, bio, nomor HP, foto profil dan sampul.
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<void> _pick(bool isCover) async {
    ...
    if (file != null && mounted) setState(() { if (isCover) { cover = file; } else { avatar = file; } });
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('social_profiles').doc(uid).set({
        'displayName': name.text.trim(), 'bio': bio.text.trim(), 'phone': phone.text.trim(),
        'photoUrl': photoUrl, 'coverPhotoUrl': coverUrl, 'updatedAt': FieldValue.serverTimestamp(),
      }, SetOptions(merge: true));
      await FirebaseFirestore.instance.collection('users').doc(uid).set({'displayName': name.text.trim(), 'phone': phone.text.trim(), 'updatedAt': FieldValue.serverTimestamp()}, SetOptions(merge: true));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        GestureDetector(onTap: () => _pick(true), child: SizedBox(height: 160, child: ClipRRect(borderRadius: BorderRadius.circular(16), child: cover != null ? Image.file(File(cover!.path), fit: BoxFit.cover) : ((profile['coverPhotoUrl'] ?? '').toString().isEmpty ? Container(color: const Color(0xFFE7EEF9), child: const Center(child: Icon(Icons.add_a_photo_outlined, size: 36))) : Image.network(profile['coverPhotoUrl'].toString(), fit: BoxFit.cover))))),
        const SizedBox(height: 14),
        Center(child: GestureDetector(onTap: () => _pick(false), child: CircleAvatar(radius: 48, backgroundImage: avatar != null ? FileImage(File(avatar!.path)) : ((profile['photoUrl'] ?? '').toString().isNotEmpty ? NetworkImage(profile['photoUrl'].toString()) : null), child: (avatar == null && (profile['photoUrl'] ?? '').toString().isEmpty) ? const Icon(Icons.person, size: 44) : null))),
  ...
  @override void dispose() { name.dispose(); bio.dispose(); phone.dispose(); super.dispose(); }
}
