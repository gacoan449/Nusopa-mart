...
class GroupService {
  ...
  Future<String> createDiscussion(String groupId, String text) async {
    final user = FirebaseAuth.instance.currentUser; if (user == null || text.trim().isEmpty) throw StateError('Sesi pengguna berakhir.');
    final profile = await db.collection('social_profiles').doc(user.uid).get(); final p = profile.data() ?? {};
    final ref = db.collection('groups').doc(groupId).collection('posts').doc();
    await ref.set({'authorId': user.uid, 'authorName': (p['displayName'] ?? user.email ?? 'Pengguna').toString(), 'authorPhotoUrl': (p['photoUrl'] ?? '').toString(), 'text': text.trim(), 'likeCount': 0, 'commentCount': 0, 'createdAt': FieldValue.serverTimestamp()}); return ref.id;
  }
